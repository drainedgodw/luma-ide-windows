...
export function registerIntelligenceIpc(getWindow: () => BrowserWindow | null) {
  ...
  const wrap = async (fn: () => Promise<unknown>) => {
    ...
    catch (e) { return { ok: false, error: { message: (e as Error).message, stderr: '' } }; }
  };
  ...
  const stashDirty = async (r: string, reason: string) => {
    ...
    const a = await dialog.showMessageBox(getWindow()!, {
      type: 'warning', buttons: ['Cancel', 'Stash changes and continue'], defaultId: 0, cancelId: 0,
      title: 'Protect local changes', message: 'The working tree contains uncommitted changes.',
    ...
    await runGit(r, ['stash', 'push', '-u', '-m', `Luma safety stash ${new Date().toISOString()}`]);
    return true;
  };
  ipcMain.handle('intel:git:commit', (_e, message: string) => wrap(async () => {
    const r = repo();
    const findings = await intel.secretScan(r);
    if (findings.length) {
      const answer = await dialog.showMessageBox(getWindow()!, {
        type: 'warning', buttons: ['Cancel commit', 'Commit anyway'], defaultId: 0, cancelId: 0,
        title: 'Secret Guard', message: `Found ${findings.length} potential secret(s)`,
  ...
  ipcMain.handle('intel:git:rewind', (_e, mode: 'soft' | 'hard', ref: string) => wrap(async () => {
    const r = repo();
    let stashed = false;
    if (mode === 'hard') stashed = await stashDirty(r, 'Hard rollback replaces the working tree.');
  ...
  ipcMain.handle('intel:invoke', (_e, method: string, ...args: unknown[]) => {
    const r = repo();
    switch (method) {
      case 'trustStatus': return wrap(() => intel.trustStatus(r));
      case 'setTrust': return wrap(() => intel.setTrust(r, args[0] as boolean));
      case 'tasks': return wrap(() => intel.tasks(r));
      case 'runTask': return wrap(() => intel.runTask(r, args[0] as intel.TaskDef));
      case 'riskMap': return wrap(() => intel.riskMap(r));
      case 'secretScan': return wrap(() => intel.secretScan(r));
      case 'preview': return wrap(() => intel.preview(r, args[0] as 'merge' | 'rebase' | 'reset', args[1] as string));
      case 'symbols': return wrap(() => intel.symbols(r, args[0] as string));
      case 'stackToolStatus': return wrap(() => stackToolStatus(r));
      case 'stackToolAction': return wrap(async () => {
        const action = args[0];
        if (action !== 'install' && action !== 'uninstall') throw new Error('Unknown Stack package action');
      ...
      case 'saveCapsule': return wrap(() => intel.saveCapsule(r, args[0] as never));
      ...
      default: return Promise.resolve({ ok: false, error: { message: 'Unknown intelligence method', stderr: '' } });
    }
  });
}
