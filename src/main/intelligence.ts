...
import { runStackToolAction } from './stackTools';
...
export interface Capsule {
  id: string; name: string; branch?: string; commit?: string; tabs: string[];
  active?: string | null; terminalOpen: boolean; note: string; at: number;
}
...
export async function tasks(repo: string): Promise<TaskDef[]> {
  ...
  try {
    ...
    for (const name of Object.keys(pkg.scripts ?? {}))
      result.push({ id: `npm:${name}`, label: `npm · ${name}`, command: 'npm', args: ['run', name] });
  } catch {}
...
async function workspaceRevision(repo: string) {
  ...
  return result.code === 0 && result.stdout.trim() ? result.stdout.trim() : `workspace:${basename(repo)}`;
}
...
export async function installTool(repo: string, packId: string, name: string): Promise<{ ok: boolean; output: string }> {
  ...
  const result = await runStackToolAction(repo, 'install', packId, name);
  return { ok: result.ok, output: result.output };
}
export async function riskMap(repo: string) {
  const raw = await runGit(repo, ['log', '--all', '--max-count=200', '--format=@@%H', '--numstat']),
    result: Record<string, { files: number; churn: number; score: number; test?: 'pass' | 'fail' }> = {};
  let hash = '';
...
export async function secretScan(repo: string) {
  const diff = await runGit(repo, ['diff', '--cached', '--no-color']),
    filenames = (await runGit(repo, ['diff', '--cached', '--name-only'])).split('\n').filter(Boolean);
...
export async function preview(repo: string, kind: 'merge' | 'rebase' | 'reset', ref: string) {
  ...
  if (kind === 'reset')
    return { kind, summary: `${await runGit(repo, ['log', '--oneline', `${ref}..HEAD`])}\n${await runGit(repo, ['diff', '--stat', `${ref}..HEAD`])}`.slice(0, 30000) };
  if (kind === 'rebase') {
...
export async function symbols(repo: string, word: string) {
  ...
  return (await searchWorkspace(repo, word)).filter((match) => expression.test(match.text)).slice(0, 200);
}
export async function rename(repo: string, from: string, to: string) {
  if (!(await trustStatus(repo))) throw new Error('Trust this workspace before textual replacement');
